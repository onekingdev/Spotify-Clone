...
const SideBar = (props) => {
  ...
  const [sidebarWidth, setSidebarWidth] = useState(197.6);
      ...
      if (isResizing) {
        setSidebarWidth(
          mouseMoveEvent.clientX -
            sidebar_ref.current.getBoundingClientRect().left
        );
      }
    },
    [isResizing]
  ...
  function renderUserPlaylist() {
    ...
    if (items.length > 0) {
      return items.map((item, index) => {
        return (
          <Link to={`/view/playlist/${item.id}`} key={index}>
            <p
              className="font-medium hover:text-white hover:cursor-pointer"
              id={item.id}
            >
              {item.name}
            </p>
  ...
  const get_liked_link = () => {};
  return (
    <div className="flex flex-row justify-between bg-black h-screen">
      <div
        className="flex flex-col text-sm py-6 w-limit mx-6"
        ref={sidebar_ref}
        ...
        <div className="flex flex-col gap-8">
          <Link to="/">
            <svg
              ...
              className="fill-white h-[90%] w-[90%]"
            >
              <g fillRule="evenodd">
          ...
          <div className="flex flex-col gap-4">
            <SidebarOption title="Home" Icon={HomeOutlinedIcon} path="" />
            <SidebarOption title="Search" Icon={SearchIcon} path="search" />
            <SidebarOption
              title="Your Library"
          ...
          <SidebarOption
            title="Liked Songs"
            Icon={FavoriteIcon}
            path={"view/liked_song/user"}
          />
        </div>
        <hr className="divider-line my-4" />
        <div className="flex flex-col overflow-y-auto gap-4 text-gray-400 h-[43%]">
          {renderUserPlaylist()}
        </div>
      </div>
...
export default connect(mapStateToProps, { get_user_playlist })(SideBar);
